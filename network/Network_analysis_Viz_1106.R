...
coRnetwork <- function(matrix, cor.cutoff, p.cutoff,name="corr") {
  ...
  pair.r <- reshape2::melt(ma, na.rm = TRUE, value.name = "cor")
  pair.r<-pair.r[which(pair.r[,3]!=0),]
  pair.r1<-pair.r[which(pair.r[,3] >= cor.cutoff),]
  pair.r2<-pair.r[which(pair.r[,3] <= (-cor.cutoff)),]
  pair.r3<-pair.r[which(pair.r[,3] <= (-cor.cutoff) | pair.r[,3] >= cor.cutoff),]
  #write.csv(pair.r, file = paste("PosNeg.",name,"all.csv",sep=""),quote = F,row.names = F)
  write.csv(pair.r1, file = paste("Pos.",name,".csv",sep=""),quote = F, row.names = F)
  write.csv(pair.r2, file = paste("Neg.",name,".csv",sep=""),quote = F,row.names = F)
  write.csv(pair.r3, file = paste("PosNeg.",name,".csv",sep=""),quote = F,row.names = F)
  ...
  mp<-matrix.cor.p
  mp[upper.tri(mp, diag = TRUE)] <- NA
  pair.p <- reshape2::melt(mp, na.rm = TRUE, value.name = "p")
  pair.p3<-pair.p[which(pair.r[,3] <= (-cor.cutoff) | pair.r[,3] >= cor.cutoff),]
  pair.r3$p<-pair.p3
  write.csv(pair.r3, file = paste("PosNeg.",name,".p.csv",sep=""),quote = F, row.names = F)
...
Abu<- read.table("THFG40.txt",header = T)
Abu <- as.matrix(Abu)

### Filtering OTUs
table <- Abu
table[table > 0] <- 1
table.generalist <- Abu[which(rowSums(table) >= 4), ]
...
pattern <- coRnetwork(Abu, 0.6, 0.9)
